    ...
    with open(labelsfile, 'r') as lf:
        ...
        with open(notes_file, 'r') as notesfile:
            with open(outfilename, 'w') as outfile:
                ...
                for i, (cur_subj, cur_labels, cur_hadm) in enumerate(labels_gen):
                    if i % 10000 == 0:
                        print(str(i) + " done")
                    subj_id, text, hadm_id = next(notes_gen)
                    while cur_hadm != hadm_id:
                        subj_id, text, hadm_id = next(notes_gen)
                    w.writerow(
                        [subj_id, str(hadm_id), text, ';'.join(cur_labels)])
    return outfilename
...
def split_data(labeledfile, base_name):
    ...
    train_file.write(
        ','.join(['SUBJECT_ID', 'HADM_ID', 'TEXT', 'LABELS']) + "\n")
    dev_file.write(
        ','.join(['SUBJECT_ID', 'HADM_ID', 'TEXT', 'LABELS']) + "\n")
    test_file.write(
        ','.join(['SUBJECT_ID', 'HADM_ID', 'TEXT', 'LABELS']) + "\n")
